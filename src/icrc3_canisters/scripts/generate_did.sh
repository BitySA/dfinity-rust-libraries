...
defaultpath="canisters/$1/api"
did_path="${outpath:-$defaultpath}"
if [[ $dryrun -eq 1 ]]; then
  echo -e "This would be written to ${did_path}/${1}.did :\n"
  candid-extractor "canisters/$1/target/wasm32-unknown-unknown/release/${1}.wasm"
else
  candid-extractor "canisters/$1/target/wasm32-unknown-unknown/release/${1}.wasm" > $did_path/can.did
fi
